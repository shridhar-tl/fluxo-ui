...
const sliderProps = {
    ...
    marks: { type: 'SliderMark[] | boolean', description: 'Display marks on the track. Pass true for auto-generated marks or an array for custom marks.' },
    ...
    onChangeEnd: { type: '(value: number | [number, number]) => void', description: 'Called when interaction ends (pointer up or keyboard).' },
};

const features: FeatureItem[] = [
    { title: 'Range Mode', description: 'Dual-thumb slider for selecting value ranges with independent low/high controls.', icon: 'M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5' },
    { title: 'Five Sizes', description: 'Extra small to extra large with proportional track, thumb, and font scaling.', icon: 'M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9 15M20.25 3.75h-4.5m4.5 0v4.5m0-4.5L15 9m5.25 11.25h-4.5m4.5 0v-4.5m0 4.5L15 15' },
    { title: 'Six Variants', description: 'Semantic color variants: default, primary, success, warning, danger, and info.', icon: 'M4.098 19.902a3.75 3.75 0 005.304 0l6.401-6.402M6.75 21A3.75 3.75 0 013 17.25V4.125C3 3.504 3.504 3 4.125 3h5.25c.621 0 1.125.504 1.125 1.125v4.072M6.75 21a3.75 3.75 0 003.75-3.75V8.197M6.75 21h13.125c.621 0 1.125-.504 1.125-1.125v-5.25c0-.621-.504-1.125-1.125-1.125h-4.072' },
    { title: 'Marks & Labels', description: 'Auto-generated or custom marks with labels along the track for precise selection.', icon: 'M3.75 12h16.5m-16.5 3.75h16.5M3.75 19.5h16.5M5.625 4.5h12.75a1.875 1.875 0 010 3.75H5.625a1.875 1.875 0 010-3.75z' },
    { title: 'Grid Snap', description: 'Snap to steps or a custom grid with configurable animation duration.', icon: 'M3.375 19.5h17.25m-17.25 0a1.125 1.125 0 01-1.125-1.125M3.375 19.5h7.5c.621 0 1.125-.504 1.125-1.125m-9.75 0V5.625m0 12.75v-1.5c0-.621.504-1.125 1.125-1.125m18.375 2.625V5.625m0 12.75c0 .621-.504 1.125-1.125 1.125m1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125' },
    { title: 'Accessibility', description: 'Full keyboard support with arrow keys, Page Up/Down, Home/End, and ARIA attributes.', icon: 'M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z' },
];
    ...
    return (
        <PageLayout sectionNavItems={sectionNavItems}>
            ...
            <section id="grid-snap" className="scroll-mt-8">
                ...
                <p className={cn('mb-4 text-sm', { 'text-gray-400': isDark, 'text-gray-500': !isDark })}>
                    Use <code>snap</code> for step-based snapping or <code>gridStep</code> for coarser grid alignment with animated transitions.
                </p>
                <GridSnap />
            </section>
            ...
            <section id="import" className="scroll-mt-8">
                ...
                <CodeBlock code={`import { Slider } from 'ether-ui';\nimport type { SliderProps, SliderMark, SliderSize, SliderVariant } from 'ether-ui';`} />
            </section>
