...
const sortableProps = {
    ...
    onChange: { type: '(items: T[], args?: any, event?: SortableChangeEvent) => void', required: true, description: 'Callback when items are reordered or changed' },
    onDrop: { type: '(source: DragItem, target: DropResult, args?: any) => void', description: 'Callback when an external item is dropped' },
    onRemove: { type: '(removed: { index: number; id?: string | number }, dropResult: DropResult | null) => void', description: 'Callback when an item is removed (dragged out)' },
    className: { type: 'string', description: 'Additional CSS classes' },
    children: { type: '(item: T, index: number, refs: { draggable?: DraggableRenderProps; droppable?: DroppableRenderProps }) => ReactNode', required: true, description: 'Render function for each item' },
};
...
const features: FeatureItem[] = [
    { title: 'Reordering', description: 'Drag items within a list to reorder them with smooth visual feedback', icon: 'M3.75 6.75h16.5M3.75 12h16.5M12 17.25h8.25' },
    { title: 'Multi-List Transfer', description: 'Move items between multiple Sortable lists for kanban-style boards', icon: 'M2.25 7.125C2.25 6.504 2.754 6 3.375 6h6c.621 0 1.125.504 1.125 1.125v3.75c0 .621-.504 1.125-1.125 1.125h-6a1.125 1.125 0 0 1-1.125-1.125v-3.75ZM14.25 8.625c0-.621.504-1.125 1.125-1.125h5.25c.621 0 1.125.504 1.125 1.125v8.25c0 .621-.504 1.125-1.125 1.125h-5.25a1.125 1.125 0 0 1-1.125-1.125v-8.25ZM3.75 16.125c0-.621.504-1.125 1.125-1.125h5.25c.621 0 1.125.504 1.125 1.125v2.25c0 .621-.504 1.125-1.125 1.125h-5.25a1.125 1.125 0 0 1-1.125-1.125v-2.25Z' },
    { title: 'Type Restrictions', description: 'Restrict which item types each Sortable accepts using itemType and accept props', icon: 'M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z' },
    { title: 'Custom Drag Handles', description: 'Use provideDragRef to attach the drag ref to a specific handle element', icon: 'M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9 15M20.25 3.75h-4.5m4.5 0v4.5m0-4.5L15 9m5.25 11.25h-4.5m4.5 0v-4.5m0 4.5L15 15' },
    { title: 'Placeholder Drop Zone', description: 'Show a placeholder at the end of the list to indicate the drop target', icon: 'M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3' },
    { title: 'Render Props', description: 'Full access to dragging state for per-item visual feedback via render function', icon: 'M6.75 7.5l3 2.25-3 2.25m4.5 0h3m-9 8.25h13.5A2.25 2.25 0 0 0 21 18V6a2.25 2.25 0 0 0-2.25-2.25H5.25A2.25 2.25 0 0 0 3 6v12a2.25 2.25 0 0 0 2.25 2.25Z' },
...
const SortablePage: React.FC = () => {
    ...
    return (
        <PageLayout sectionNavItems={sectionNavItems}>
            <div>
                <h1 className={cn('text-2xl md:text-4xl font-bold mb-4', { 'text-gray-100': isDark, 'text-gray-900': !isDark })}>Sortable</h1>
            ...
            <section id="basic" className="scroll-mt-8">
                <h2 className={cn('text-2xl font-semibold mb-4', { 'text-gray-100': isDark, 'text-gray-900': !isDark })}>Basic Sortable List</h2>
                <BasicSortable />
            </section>

            <section id="complex-items" className="scroll-mt-8">
                <h2 className={cn('text-2xl font-semibold mb-4', { 'text-gray-100': isDark, 'text-gray-900': !isDark })}>Complex Items with Details</h2>
                <ComplexItems />
            </section>

            <section id="multiple-lists" className="scroll-mt-8">
                <h2 className={cn('text-2xl font-semibold mb-4', { 'text-gray-100': isDark, 'text-gray-900': !isDark })}>Multiple Sortable Lists</h2>
                <MultipleLists />
            </section>
            ...
            <section id="drag-handles" className="scroll-mt-8">
                <h2 className={cn('text-2xl font-semibold mb-4', { 'text-gray-100': isDark, 'text-gray-900': !isDark })}>Custom Drag Handles</h2>
                <DragHandles />
            </section>

            <section id="import" className="scroll-mt-8">
                <h2 className={cn('text-2xl font-semibold mb-4', { 'text-gray-100': isDark, 'text-gray-900': !isDark })}>Import</h2>
                <CodeBlock code={`import { DragDropProvider, Sortable } from 'ether-ui/dnd';`} />
            </section>
