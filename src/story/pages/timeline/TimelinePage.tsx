...
const timelineProps = {
    ...
    connectorStyle: { type: "'solid' | 'dashed' | 'dotted'", default: "'solid'", description: 'Style of the connector lines between events.' },
...
const features: FeatureItem[] = [
    { title: 'Vertical & Horizontal', description: 'Two layout modes for different use cases: vertical scrolling or horizontal milestone display.', icon: 'M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25z' },
    { title: 'Alternate Alignment', description: 'Events can alternate between left and right for a visually balanced layout.', icon: 'M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5' },
    { title: 'Custom Markers', description: 'Use icons, custom elements, or the default dot as timeline markers.', icon: 'M9.53 16.122a3 3 0 00-5.78 1.128 2.25 2.25 0 01-2.4 2.245 4.5 4.5 0 008.4-2.245c0-.399-.078-.78-.22-1.128z' },
    { title: 'Color Variants', description: 'Five color options: primary, success, warning, danger, and info.', icon: 'M4.098 19.902a3.75 3.75 0 005.304 0l6.401-6.402M6.75 21A3.75 3.75 0 013 17.25V4.125C3 3.504 3.504 3 4.125 3h5.25c.621 0 1.125.504 1.125 1.125v4.072M6.75 21a3.75 3.75 0 003.75-3.75V8.197' },
    { title: 'Connector Styles', description: 'Solid, dashed, or dotted connector lines between timeline markers.', icon: 'M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25H12' },
    { title: 'Accessibility', description: 'Semantic list markup with ARIA roles for screen reader support.', icon: 'M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z' },
];
...
const TimelinePage: React.FC = () => {
    ...
    return (
        <PageLayout sectionNavItems={sectionNavItems}>
            <div>
                <h1 className={cn('text-2xl md:text-4xl font-bold mb-4', { 'text-gray-100': isDark, 'text-gray-900': !isDark })}>Timeline</h1>
            ...
            <section id="alternate" className="scroll-mt-8">
                <h2 className={cn('text-2xl font-semibold mb-4', { 'text-gray-100': isDark, 'text-gray-900': !isDark })}>Alternate Alignment</h2>
                <p className={cn('mb-4 text-sm', { 'text-gray-400': isDark, 'text-gray-500': !isDark })}>
            ...
            <section id="import" className="scroll-mt-8">
                ...
                <CodeBlock code={`import { Timeline } from 'ether-ui';\nimport type { TimelineEvent, TimelineProps } from 'ether-ui';`} />
            </section>
            ...
            <section id="event-props" className="scroll-mt-8">
                <h2 className={cn('text-2xl font-semibold mb-4', { 'text-gray-100': isDark, 'text-gray-900': !isDark })}>TimelineEvent Interface</h2>
