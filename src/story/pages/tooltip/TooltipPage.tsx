...
const features: FeatureItem[] = [
    { title: 'Global Manager Pattern', description: 'Mount TooltipManager once at the app root and call showTooltip/hideTooltip from any event handler without prop drilling.', icon: 'M12 21a9.004 9.004 0 0 0 8.716-6.747M12 21a9.004 9.004 0 0 1-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 0 1 7.843 4.582M12 3a8.997 8.997 0 0 0-7.843 4.582m15.686 0A11.953 11.953 0 0 1 12 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0 1 21 12c0 .778-.099 1.533-.284 2.253M3.284 14.253A8.959 8.959 0 0 1 3 12c0-1.306.278-2.546.777-3.668' },
    { title: 'Smart Auto Placement', description: "The 'auto' placement mode analyzes available viewport space and picks the best corner so tooltips never clip off-screen.", icon: 'M9 6.75V15m6-6v8.25m.503 3.498 4.875-2.437c.381-.19.622-.58.622-1.006V4.82c0-.836-.88-1.38-1.628-1.006l-3.869 1.934c-.317.159-.69.159-1.006 0L9.503 3.252a1.125 1.125 0 0 0-1.006 0L3.622 5.689C3.24 5.88 3 6.27 3 6.695V19.18c0 .836.88 1.38 1.628 1.006l3.869-1.934c.317-.159.69-.159 1.006 0l4.994 2.497c.317.158.69.158 1.006 0Z' },
    { title: 'Rich Content Support', description: 'Pass any React node as tooltip content — formatted text, icons, lists, or custom JSX — not just plain strings.', icon: 'M17.25 6.75 22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3-4.5 16.5' },
    { title: 'Configurable Hide Delay', description: 'Set a per-tooltip timeout for how long after mouse-leave the tooltip stays visible — from instant (0 ms) to several seconds.', icon: 'M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z' },
    { title: 'Hover-Safe Tooltip', description: 'Moving the mouse from the trigger onto the tooltip itself cancels the hide timer, allowing users to interact with rich content.', icon: 'M15.042 21.672 13.684 16.6m0 0-2.51 2.225.569-9.47 5.227 7.917-3.286-.672ZM12 2.25V4.5m5.834.166-1.591 1.591M20.25 10.5H18M7.757 14.743l-1.59 1.59M6 10.5H3.75m4.007-4.243-1.59-1.59' },
    { title: 'Works on Any Element', description: 'Attach onMouseEnter/onMouseLeave to buttons, icons, text spans, table cells, or any DOM element — no wrapper component needed.', icon: 'M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25H12' },
    { title: 'Portal Rendering', description: 'Tooltips render in a portal above all other content, ensuring they never get clipped by overflow:hidden containers.', icon: 'M13.5 6H5.25A2.25 2.25 0 0 0 3 8.25v10.5A2.25 2.25 0 0 0 5.25 21h10.5A2.25 2.25 0 0 0 18 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25' },
...
const TooltipPage: React.FC = () => {
    ...
    return (
        <PageLayout sectionNavItems={sectionNavItems}>
            <div>
                <h1 className={cn('text-2xl md:text-4xl font-bold mb-4', { 'text-gray-100': isDark, 'text-gray-900': !isDark })}>Tooltip</h1>
            ...
            <section className="scroll-mt-8" id="rich-content">
                <h2 className={cn('text-2xl font-semibold mb-4', { 'text-gray-100': isDark, 'text-gray-900': !isDark })}>Rich Content (JSX)</h2>
                <RichContent />
            </section>
            ...
            <section className="scroll-mt-8" id="import">
                ...
                <CodeBlock code={`import { TooltipManager, showTooltip, hideTooltip } from 'ether-ui';`} />
            </section>
