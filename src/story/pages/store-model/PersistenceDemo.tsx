...
const persistKey = 'ether-ui-demo-persist-counter';
...
const PersistedCounter: React.FC<PersistedCounterProps> = ({ store }) => {
    ...
    const state = useCounter() as CounterState & { id: string; isStale: boolean; isLoading: boolean; isSaving: boolean; isDeleting: boolean };
    ...
    return (
        <div className={cn('rounded-lg border p-6 text-center', {
            ...
            <p className={cn('text-xs mb-2 font-medium', { 'text-gray-500': isDark, 'text-gray-400': !isDark })}>
                {state.label}
            </p>
            ...
            <div className={cn('text-xs text-center px-4 py-2 rounded-lg mb-4 max-w-sm mx-auto', {
                ...
                State auto-saves to localStorage via <code className="font-mono font-semibold">saveOnChange</code>.
                The <code className="font-mono font-semibold">persist</code> callback writes data, and <code className="font-mono font-semibold">loadFromPersist</code> restores it on creation.
                Try refreshing the page or unmounting below.
            </div>
            <div className="flex gap-2 justify-center flex-wrap">
                ...
                <Button
                    ...
                    size="sm"
                    variant="secondary"
                    onClick={() => store.setState({ count: 0, lastUpdated: Date.now() })}
                />
                <Button
                    label="Refresh Page"
                    size="sm"
...
const StorageInspector: React.FC = () => {
    ...
    return (
        <div className={cn('rounded-lg border p-4 mt-4', {
            'border-white/10 bg-white/5': isDark,
            'border-gray-200 bg-gray-50': !isDark,
        })}>
            <div className="flex items-center justify-between mb-2">
                <p className={cn('text-xs font-semibold', { 'text-gray-400': isDark, 'text-gray-500': !isDark })}>
                    localStorage Inspector
                </p>
                <Button label="Refresh" size="xs" variant="secondary" layout="plain" onClick={refresh} />
            </div>
            <pre className={cn('text-[11px] font-mono p-3 rounded overflow-x-auto whitespace-pre-wrap break-all', {
...
const PersistenceDemo: React.FC = () => {
    ...
    return (
        <>
            <ComponentDemo title="Persistence Demo" description="State persists to localStorage and survives component remount and page refresh" centered={false}>
